...
Image versions SHALL be named `major.minor.patch` with no prerelease suffix. Resolving the highest
matching version orders version tags, and that ordering places a prerelease *after* the release it
precedes rather than before it — the reverse of what a prerelease means. A prerelease tag would
therefore be selected ahead of its own release, and every subsequent chart-only release would
record the candidate instead. Excluding prereleases from the scheme is what makes "the highest
existing version" unambiguous; a scheme that admitted them would need an ordering that understands
them.
...
#### Scenario: No prerelease participates in resolution

- **WHEN** an image version is tagged
- **THEN** it names a `major.minor.patch` version carrying no prerelease suffix
- **AND** resolution of the highest matching version is unambiguous as a result, because the
  ordering used would otherwise place a prerelease ahead of the release it precedes
